...
# The app/secret.json file contains the app_secret, client_id and client_secret.
app.secret_key = json.loads(open("app/secret.json").read())["app_secret"]
...
def top_tracks():
    if request.method == "POST":
        ...
        list = sp.current_user_top_tracks(
            limit=amount_of_items, offset=0, time_range=time_range
        )["items"]
...
def get_token():
    ...
    now = int(time.time())
    is_token_expired = session.get("token_info").get("expires_at", 0) - now < 60
...
def create_spotify_oauth():
    sp_oauth = spotipy.oauth2.SpotifyOAuth(
        client_id=json.loads(open("app/secret.json").read())["client_id"],
        client_secret=json.loads(open("app/secret.json").read())["client_secret"],
